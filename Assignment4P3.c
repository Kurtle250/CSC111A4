 ...
 *  Author:      B. Bird
 *  Date:        Oct. 9, 2016
 *  Assignment:  Assignment 4 Part 3
 *  File name:   A4P3_2016_Template.c
 *  Description: Generate HTML5 and SVG code to display a test image.
 */
...
void rectangle(FILE *f, int x, int y, int width, int height, int r, int g, int b, float alpha){
	fprintf(f,"<rect x=\"%d\" y=\"%d\"  width= \"%d\" height = \"%d\" \" style = fill:rgb(%d,%d,%d) opacity:\"%f\" />",x,y,width,height,r,g,b,alpha);
} /* rectangle */
...
*/
int main(){
	FILE* f = NULL; // file pointer type defined in stdio.h

	f = fopen(HTML5_SVG_OUTPUT_FILENAME,"w");
